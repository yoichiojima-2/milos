...
4. **Internal address.** From inside a runner (`Bash: curl -sS $MILOS_API_URL/healthz`) the internal service must answer over the restricted VIP. If `run.app` does not resolve to the VIP, adjust the private zone in `modules/network`.
