...
| MC-08 | Evidence immutability: the evidence bucket is versioned and under a (lockable) retention policy; the sandbox identity has **no grant** on it | `infra/evidence.tf`; the *absence* of an IAM member in `infra/main.tf` | A.8.15, A.5.33 protection of records | 9.1 |
| MC-09 | Encryption at rest: all stores use Google-managed default encryption; CMEK is deliberately out of scope ([soa-notes.md](soa-notes.md)) | GCP default encryption (no Terraform resource) | A.8.24 cryptography | — |
| MC-10 | Data residency: Firestore, buckets, and the sandbox all pin to one configured region | `var.region`/`var.firestore_location` used on every resource | A.5.31 | — |
...
| MC-12 | Access logging: Data Access audit logs on Firestore/GCS/Run, routed to a dedicated logging bucket with its own retention — who read what, independent of app code | `infra/logging.tf` | A.8.15, A.8.16 monitoring | 9.1 |
