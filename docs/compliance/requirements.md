...
| --- | --- | --- | --- |
| REQ-D-01 | Data carries a class; the class decides project placement and access boundary | `modules/foundation` project roles; `modules/data` per class with labels and dataset IAM, agent workspaces (`agent_<id>`) inside the class's project; `AgentVersion.data_classes`, carried on every audit entry | `terraform validate` in CI; label query on data projects (procedure) |
...
| REQ-D-05 | Retention per class enforced by rule | `modules/data` lifecycle and table expiration (workspace datasets included); `warehouse.check_sql` refuses statements that set an expiration; `modules/runtime` snapshot lifecycle | `terraform validate`; rule presence; `test_bq_write_targets_only_the_workspace` |
| REQ-D-06 | One identity system; no platform-held credentials | IAP on the public API (`iap_enabled`), assertion verified in `auth.TokenVerifier`; the web console is served by that same service and calls `/v1` with the IAP cookie, so it holds no identity of its own (`GET /v1/me` only reports the verified caller) and the lease token never leaves the API (`SessionView`); the scheduler's identity token verified on the internal routes it uses; API keys only in Secret Manager | `test_public_requires_iap`, `test_console_never_shadows_the_api`, `test_public_session_never_carries_the_lease_token`, `test_scheduler_identity_is_verified`; secret scan (procedure) |
...
| REQ-D-08 | One service account per agent, traceable to the definition | `modules/runtime` `google_service_account.runner[agent]` and `google_service_account.workspace[agent]` (BigQuery, impersonated by the internal connector only); `AgentVersion.runner_sa`, `datasets`, `workspace`; `milos agents registry` | Compare `terraform output runner_service_accounts` / `workspace_service_accounts` with the registry and the definitions (procedure) |
...
| REQ-D-14 | Intended use in the definition; deviations blocked structurally | `AgentVersion.allowed_tools`, `service._outcome`; for BigQuery, `AgentVersion.datasets` and `workspace` returned as the permission's `DataScope`, enforced by IAM on the workspace identity and by `warehouse.check_plan` before a statement runs | `test_unlisted_tool_is_denied`, `test_bq_query_runs_only_selects_inside_the_scope`, `test_bq_write_targets_only_the_workspace`, `test_workspace_tables_belong_to_the_session_that_created_them`, `test_permission_lookup_carries_the_definition_scope` |
| REQ-D-15 | Dataset provenance per unit | `modules/data` dataset labels (`source`; workspaces carry `source=agent` and `agent=<id>`); BigQuery jobs labelled with session, agent and tool use id | Label query (procedure); `test_bq_query_runs_only_selects_inside_the_scope` asserts the labels |
