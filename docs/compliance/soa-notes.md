...
- **Existing-project development mode has no project separation.** `infra/envs/existing-project` reuses one project with separate runtime and egress networks, project-scoped audit retention, and explicit Google users authenticated through IAP. It has no folder controls, organization policies, or VPC Service Controls perimeter. The project owner can administer all components; this mode does not meet the organization deployment's separation requirements.
