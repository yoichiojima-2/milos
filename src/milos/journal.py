...
including the policy version and hash in force, so every audit row is evidence
of which rules governed it.
...
runner transitions (claimed, released, branch_created, incident_opened, ...).
"""
...
def event_message(event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if event.get("type") == "prompt":
        # A prompt is what the user typed; clients see it as a UserMessage.
        return {"kind": "user", "content": event["payload"].get("text") or ""}
    return None


def milos_version() -> str | None:
    try:
        from importlib.metadata import PackageNotFoundError, version

        return version("milos")
    except PackageNotFoundError:
        return None


def git_info(path: Any) -> dict[str, Any] | None:
    """Snapshot of the working directory's git state, or None when it isn't a
    repo."""
...
def build_context(
    *,
    ...
    policy_version/policy_hash ride every record so evidence can show which
    policy was in force when each message, tool call, and approval happened —
    the stamp costs nothing here and cannot be forgotten by a caller.
    """
    return {
        "cwd": cwd,
        "model": model,
        "permission_mode": permission_mode,
        "version": milos_version(),
        "workspace": workspace,
        "lease_id": lease_id,
