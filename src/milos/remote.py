...
Policy enforcement point one lives here: a new session resolves its option
layers, is checked against the active policy (`policy.check_session`), and is
stamped with the policy version + hash it was admitted under. No active policy
means no session — fail closed, since an ungoverned run cannot be evidenced.
"""
...
from .errors import MilosError, PolicyError, SessionTerminated
...
async def _prompt_texts(prompt: str | AsyncIterable[dict[str, Any]]) -> list[str]:
    ...
    async for item in prompt:
        ...
        if isinstance(content, str):
            ...
            raise MilosError(f"cannot serialize streamed prompt item: {item!r}")
    return texts
...
async def _relay_approvals(store: StoreProtocol, session_id: str, options: AgentOptions) -> None:
    ...
    The decision is recorded as decided_by="sdk" — a human-oversight record
...
async def branch_from_event(
    ...
    if lease_active(session):
        raise MilosError(f"session {session_id} is running — interrupt it before rewinding")
    event = await store.get_event(session_id, from_event)
    if event is None:
        raise MilosError(f"event {from_event} not found in session {session_id}")
...
async def require_active_policy(store: StoreProtocol) -> dict[str, Any]:
    """The active policy document, or a refusal. New sessions cannot start on
    an install that never applied a policy — apply one (the permissive example
    in docs/compliance/policy.example.yaml counts) rather than running
    ungoverned."""
    active = await store.get_active_policy()
    if active is None:
        raise PolicyError(
            "no policy is applied — run `milos policies apply <policy.yaml>` first"
            " (docs/compliance/policy.example.yaml is a permissive starter)"
        )
    return active
...
async def attach_session(store: StoreProtocol, options: AgentOptions) -> tuple[str, str, int]:
    ...
    if options.from_event and not options.resume:
        raise MilosError("from_event requires resume=<session_id>")
    if options.resume:
    ...
    # options, so a later edit to the agent never changes this session. The
    # policy judges the *merged* result — what the run would actually get —
    # and the admitted version is stamped onto the session for the runner to
    # pin its gate to.
    from . import agents

    options = await agents.resolve(store, options)
    active = await require_active_policy(store)
    policy_module.check_session(options, active["policy"])
