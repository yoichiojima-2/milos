...
discovers them; the CLI edits them like workspace files. Official Anthropic
...
in asyncio.to_thread (same contract as state.py).
"""
...
from .layout import skill_prefix, skills_root
from .names import NAME, validate_name
from .state import _bucket, delete_in_prefix, read_in_prefix, write_in_prefix
...
# Frontmatter sits at the top of SKILL.md, so the CLI reads a prefix rather
...
def _unquote(value: str) -> str:
    ...
    quotes something keeps all of it. An unterminated scalar (the CLI reads
    ...
    while index < len(value):
        ...
        if quote == '"' and char == "\\" and index + 1 < len(value):
            ...
            # backslash would turn é into a literal "u00e9" in the CLI
...
def parse_description(data: bytes) -> str | None:
    ...
    fires, so it is what the CLI shows to explain a skill. Deliberately a
...
def delete_skill(project: str, bucket_name: str, name: str, workspace: str | None = None) -> int:
    ...
    it file-by-file from the CLI would be unusable."""
...
def push(
    ...
    *,
    ...
    push writes CLI-editable. Merges by default (like artifacts.push);
...
def sync_official(
    ...
    *,
    ...
    everything the sync writes CLI-editable.
    """
...
# The per-file size cap for pushes, syncs, and reads. A fixed constant rather
# than deployment config: it exists to keep skill files CLI-readable and
# uploads bounded, not to police storage.
MAX_SKILL_FILE_BYTES = 10 * 1024 * 1024


def stats(
    project: str, bucket_name: str, workspace: str | None = None
) -> dict[str, dict[str, Any]]:
    """Every skill in a scope: name -> {files, bytes, description}.

    One listing of the scope's whole prefix; the description comes from each
    SKILL.md's frontmatter (a bounded prefix read — canvas-design's SKILL.md is
    5 MB and the description is line 3).
    """
    ...
    for blob in bucket.list_blobs(prefix=root):
        relative = blob.name[len(root) :]
        name, _, file = relative.partition("/")
        if not name or not file:
            continue
        row = rows.setdefault(name, {"files": 0, "bytes": 0, "description": None})
        row["files"] += 1
        row["bytes"] += blob.size or 0
        if file == SKILL_MD:
            targets[name] = blob.name
    for name, blob_name in targets.items():
        try:
            data = bucket.blob(blob_name).download_as_bytes(start=0, end=FRONTMATTER_BYTES - 1)
        except Exception:
            continue  # a listing race or a permissions blip costs a description, not the listing
        rows[name]["description"] = parse_description(data)
    return rows


def files(
    project: str, bucket_name: str, name: str, workspace: str | None = None
) -> list[dict[str, Any]]:
    """One skill's files: [{name, size, updated}], sorted by name."""
    prefix = skill_prefix(name, workspace)
    return sorted(
        (
