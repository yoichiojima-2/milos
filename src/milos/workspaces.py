...
class WorkspaceError(MilosError):
...
async def update(
    ...
    *,
    ...
    the first update materialises the doc."""
    options = options or AgentOptions()
    run_options.project = run_options.project or options.project
    run_options.validate()
    store = _store(options, store)
...
def read_claude_md(project: str, bucket_name: str, name: str) -> str | None:
    ...
    asyncio.to_thread (same contract as state.py)."""
    try:
        data, _content_type = state.read_in_prefix(
            project,
            bucket_name,
            layout.workspace_prefix(name),
            "workspace file",
            "CLAUDE.md",
