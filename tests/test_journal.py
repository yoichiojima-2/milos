...
from milos.journal import JournalWriter, build_context, event_message, make_event, new_branch_id
...
def test_build_context_snapshot():
    context = build_context(
        ...
        policy_version="v000001",
        policy_hash="abc",
    )
    assert context["cwd"] == "/work/ws"
    assert context["model"] == "m"
    assert context["policy_version"] == "v000001"
    assert context["policy_hash"] == "abc"
