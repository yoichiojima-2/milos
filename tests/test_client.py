...
from milos import AgentOptions, AssistantMessage, MilosClient, ResultMessage, TextBlock
...
async def make_client(store, **kwargs):
    policy = validate_policy({})
    if not store.policies:
        await store.create_policy(policy, {"hash": canonical_hash(policy)})
    client = MilosClient(AgentOptions(project="proj-1", **kwargs))
    client._store = store
    return client
...
async def test_multi_turn_conversation():
    store = FakeStore()
    async with await make_client(store) as client:
...
async def test_query_before_connect_raises():
    client = MilosClient(AgentOptions(project="proj-1"))
    with pytest.raises(MilosError, match="not connected"):
