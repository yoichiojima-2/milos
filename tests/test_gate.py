...
from milos.gate import Gate, call_hash
from milos.journal import JournalWriter
from milos.policy import validate_policy
from milos.types import PermissionResultAllow, PermissionResultDeny
...
def make_gate(store, policy=PERMISSIVE, **kwargs):
    writer = JournalWriter(store, SID, branch="main", seq=0, tip_uuid=None)
    return Gate(store, SID, writer, policy=policy, **kwargs)
...
async def test_policy_denied_tool_is_refused_and_audited():
    store = await make_store()
    gate = make_gate(store, policy=validate_policy({"tools": {"deny": ["Bash"]}}))
    result = await gate._pre_tool_use(hook_input(), "t1", None)
    assert result["hookSpecificOutput"]["permissionDecision"] == "deny"
    assert result["hookSpecificOutput"]["permissionDecisionReason"] == "denied by policy"
    (row,) = await store.list_tool_calls(SID)
    assert row["decision"] == "policy_denied"
    # a tool the policy doesn't name still passes
    assert await gate._pre_tool_use(hook_input("Read"), "t2", None) == {}


async def test_policy_approval_rule_forces_ask():
    store = await make_store()
    policy = validate_policy({"require_approval": [{"tools": ["Bash"], "reason": "shell"}]})
    gate = make_gate(store, policy=policy)
    result = await gate._pre_tool_use(hook_input(), "t1", None)
    # "ask" makes the SDK fall through to can_use_tool even under a
    # permission_mode that would otherwise skip it.
    assert result["hookSpecificOutput"]["permissionDecision"] == "ask"
    assert result["hookSpecificOutput"]["permissionDecisionReason"] == "shell"
    (row,) = await store.list_tool_calls(SID)
    assert row["decision"] == "policy_ask"


async def test_approval_request_carries_policy_reason():
    store = await make_store()
    policy = validate_policy({"require_approval": [{"tools": ["Bash"], "reason": "shell"}]})
    gate = make_gate(store, policy=policy, approval_timeout=0.05, poll_interval=0.01)
    await gate.can_use_tool("Bash", {"command": "x"}, None)
    approval = await store.get_approval(SID, call_hash("Bash", {"command": "x"}))
    assert approval["reason"] == "shell"
